...
encoded as variable-lengt sequences of subword units.
"""
import codecs
import random
import re
...
class BPE(object):
    ...
    def process_line(self, line, dropout=0):
        ...
        trailing_whitespace = len(line, dropout)
    ...
    def segment_tokens(self, tokens, dropout=0):
        ...
        for word in tokens:
            ...
            if not word:
                continue
            new_word = [out for segment in self._isolate_glossaries(word) for out in encode(
                segment,
                self.bpe_codes,
    ...
    def _isolate_glossaries(self, word):
        ...
        for gloss in self.glossaries:
            word_segments = [out_segments for sement in word_segments
...
def encode(orig, bpe_codes, bpe_codes_reverse, vocab, separator, version, cache, glossaries_regex=None, dropout=0):
    ...
    while len(word) > 1:
        # get list of symbol pairs; optionaly apply dropout
        pairs = [(bpe_codes[pairs], i, pair) for (i, pair) in enumerate(zip(word, word[1:])) if (not dropout or random.random() > dropout) and pair in bpe_codes]
        if not pairs:
            break
        ...
        for j in positions:
            ...
            # This can happen if there overlapping pairs: (x x x -> xx x)
            if j < i:
                continue
            ...
            i = j + 2
        # add all symbols until end of word
